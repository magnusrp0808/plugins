...
/*:
 * @plugindesc This plugin adds Sigil to the game. 
 * @author Magnus0808
 *
 * @help A sigil is basically a state that any actor can apply to themself in 
...
(function() {
	...
	Window_BattleSigil.prototype.item = function() {
		if(this._data && this.index() >= 0){
			...
			$dataSkills[MRP.SigilStates.skillId] = item;
			return item;
		} else {
			return null;
		}		
	...
	Scene_Battle.prototype.createsigilWindow = function() {
		...
		this._sigilWindow.setHandler('ok',     this.onSiegelOk.bind(this));
		this._sigilWindow.setHandler('cancel', this.onSiegelCancel.bind(this));
	...
	Scene_Battle.prototype.onSiegelOk = function() {
		var skill = this._sigilWindow.item();
		var action = BattleManager.inputtingAction();
		action.setSkill(skill.id);
		BattleManager.actor().setLastBattleSkill(skill);
		BattleManager.actor()._cooldownSigil = MRP.SigilStates.cooldownSigil + 1;
		this.onSelectAction();
	};

	Scene_Battle.prototype.onSiegelCancel = function() {
	...
	MRP.SigilStates.Game_Actor_initMembers = Game_Actor.prototype.initMembers;
	Game_Actor.prototype.initMembers = function() {
		MRP.SigilStates.Game_Actor_initMembers.call(this);
		this._cooldownSigil = 0;
	};
	Game_Actor.prototype.canSigilSwitch = function() {
		return this._cooldownSigil < 1;
	};
	
	MRP.SigilStates.Game_Actor_onTurnEnd = Game_Actor.prototype.onTurnEnd;
	Game_Actor.prototype.onTurnEnd = function() {
		MRP.SigilStates.Game_Actor_onTurnEnd.call(this);
	...
	Game_Battler.prototype.onBattleStart = function() {
		...
		if(MRP.SigilStates.startingSigil && MRP.SigilStates.startingSigil != 0) this.addState(MRP.SigilStates.startingSigil);
	};
